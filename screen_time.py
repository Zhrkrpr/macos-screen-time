...
def print_power_status():
    ...
    print()
    print("Час роботи від акумулятора розраховується лише для сеансів роботи від акумулятора.")
...
def print_battery_session(battery_seconds, screen_on_seconds, screen_off_seconds):
    print(SEPARATOR)
    print("          Battery Session")
    print(SEPARATOR)
    print()
    print(f"Battery Time:        {format_duration(battery_seconds)}")
    print(f"Screen On Time:       {format_duration(screen_on_seconds)}")
    print(f"Screen Off Time:     {format_duration(screen_off_seconds)}")
    print()
    print(SEPARATOR)
